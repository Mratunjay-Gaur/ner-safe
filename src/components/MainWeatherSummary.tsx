    ...
    >
      <div>
        ...
        <div className="flex items-center justify-between gap-2">
          {isStale ? (
            <div className="inline-flex items-center gap-1.5 bg-amber-50 text-amber-700 border border-amber-200/80 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider">
              ...
              {t('weather.cachedStale', 'CACHED / STALE')}
            </div>
          ) : (
        ...
        <div className="mt-2.5">
          ...
          <p className="text-xs text-slate-500 font-medium flex items-center gap-1 mt-0.5">
            ...
            <span>{location.state}, {t('location.india', 'India')}</span>
          </p>
        </div>
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <div>
              ...
              <span className="text-[11px] text-slate-500 font-medium">
                {t('weather.feelsLike', 'Feels like')} <strong className="text-slate-700 font-semibold">{current.apparentTemperature}°C</strong>
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-slate-100 pt-3 mt-3">
          <div className="text-[10px] uppercase font-bold text-slate-400 tracking-wider mb-2">
            {t('weather.currentObservations', 'Current Observations')}
          </div>
          <div className="grid grid-cols-2 gap-2.5 text-xs">
            <div className="flex flex-col bg-slate-50/60 p-2 rounded-lg border border-slate-100">
              <span className="text-[10px] text-slate-400 uppercase font-bold">{t('weather.humidity', 'Humidity')}</span>
              <span className="font-bold text-slate-900 text-sm mt-0.5">{current.relativeHumidity}%</span>
            </div>

            <div className="flex flex-col bg-slate-50/60 p-2 rounded-lg border border-slate-100">
              <span className="text-[10px] text-slate-400 uppercase font-bold">{t('weather.wind', 'Wind')}</span>
              <span className="font-bold text-slate-900 text-sm mt-0.5">{current.windSpeed} km/h</span>
            </div>

            <div className="flex flex-col bg-slate-50/60 p-2 rounded-lg border border-slate-100">
              <span className="text-[10px] text-slate-400 uppercase font-bold">{t('weather.precipitation', 'Precipitation')}</span>
              <span className="font-bold text-slate-900 text-sm mt-0.5">{current.precipitation} mm</span>
            </div>

            <div className="flex flex-col bg-slate-50/60 p-2 rounded-lg border border-slate-100">
              <span className="text-[10px] text-slate-400 uppercase font-bold">{t('weather.pressure', 'Pressure')}</span>
              <span className="font-bold text-slate-900 text-sm mt-0.5">{current.surfacePressure} hPa</span>
            </div>

            <div className="flex flex-col bg-slate-50/60 p-2 rounded-lg border border-slate-100">
              <span className="text-[10px] text-slate-400 uppercase font-bold">{t('weather.visibility', 'Visibility')}</span>
              <span className="font-bold text-slate-900 text-sm mt-0.5">{current.visibility} km</span>
            </div>

            <div className="flex flex-col bg-slate-50/60 p-2 rounded-lg border border-slate-100">
              <span className="text-[10px] text-slate-400 uppercase font-bold">{t('weather.cloudCover', 'Cloud Cover')}</span>
      ...
      <div className="mt-4 pt-2.5 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-400">
        <span className="flex items-center gap-1">
          ...
          <span>{t('weather.sync', 'Sync:')} {formatTime(current.updatedAt)}</span>
        </span>
